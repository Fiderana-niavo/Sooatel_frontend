...
import { Bed, Edit, Trash2, Plus, X, Check } from "lucide-react";
...
export function RoomsModal({ isOpen, onClose, data, onAdd, onEdit, onDelete }: RoomsModalProps) {
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <DialogTitle className="text-2xl font-bold tracking-tight text-secondary">
                  Gestion: Chambres/Salles
                </DialogTitle>
                <DialogDescription className="text-muted-foreground mt-1 text-sm">
                  Gérez les chambres et salles disponibles.
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
                ...
                >
                  {editingId === item.idRoom ? (
                    ...
                    <div className="flex-1 font-semibold text-foreground text-sm">
                      {item.label || item.eventName || item.roomNumber || item.ref || item.idRoom}
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {editingId === item.idRoom ? (
                      ...
                      <>
                        ...
                        <Button size="icon" variant="ghost" onClick={() => onDelete(item.idRoom)} className="opacity-0 group-hover:opacity-100 text-destructive">
