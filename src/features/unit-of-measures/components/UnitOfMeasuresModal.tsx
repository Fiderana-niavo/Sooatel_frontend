...
export function UnitOfMeasuresModal({ isOpen, onClose, data, onAdd, onEdit, onDelete }: UnitOfMeasuresModalProps) {
          ...
          <div className="bg-muted/10 p-5 rounded-2xl border border-border/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
              <div className="space-y-1.5">
                ...
                <Input placeholder="Ex: Kilogramme" value={newLabel} onChange={(e) => setNewLabel(e.target.value)} className="bg-background" />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-muted-foreground uppercase">Symbole</label>
                <Input placeholder="Ex: kg" value={newSymbol} onChange={(e) => setNewSymbol(e.target.value)} className="bg-background" />
              </div>
          ...
          <div className="space-y-3 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
            {filteredData.length === 0 ? (
              ...
              filteredData.map((item: any) => (
                <div key={item.idUnit} className="p-4 rounded-xl border bg-card hover:bg-muted/30 transition-colors group">
                  {editingId === item.idUnit ? (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
                      ...
                      <Input value={editSymbol} onChange={(e) => setEditSymbol(e.target.value)} className="h-9" placeholder="Symbole" />
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="text-sm text-muted-foreground flex-1 truncate">{item.symbol || "-"}</div>
                      </div>
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <Button size="icon" variant="ghost" onClick={() => onDelete(item.idUnit)} className="opacity-0 group-hover:opacity-100 text-destructive"><Trash2 className="size-4" /></Button>
                      </div>
                    </div>
                  )}
                </div>
