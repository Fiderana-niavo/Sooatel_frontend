...
import { BedDouble, Edit, Trash2, Plus, X, Check } from "lucide-react";
...
interface RoomTypesModalProps {
  ...
  onClose: () => void;
  data: RoomType[];
  onAdd: (data: Partial<RoomType>) => void;
  onEdit: (id: string, data: Partial<RoomType>) => void;
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <DialogTitle className="text-2xl font-bold tracking-tight text-secondary">
                  Gestion: Types de Chambres
                </DialogTitle>
                <DialogDescription className="text-muted-foreground mt-1 text-sm">
                  Définissez les catégories de chambres.
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
                ...
                >
                  {editingId === item.idRoomType ? (
                    ...
                    <div className="flex-1 font-semibold text-foreground text-sm">
                      {item.label || item.eventName || item.roomNumber || item.ref || item.idRoomType}
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {editingId === item.idRoomType ? (
                      ...
                      <>
                        ...
                        <Button size="icon" variant="ghost" onClick={() => onDelete(item.idRoomType)} className="opacity-0 group-hover:opacity-100 text-destructive">
